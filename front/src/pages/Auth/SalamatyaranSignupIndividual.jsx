...
const SalamatyaranSignupIndividual = () => {
  ...
  useEffect(() => {
    ...
    if (profileType !== "individual" || !step1?.first_name) {
  ...
  useEffect(() => {
    ...
    setJobAddress(saved.job_address || saved.jobAddress || "");
    setCollaborationType(
      findLookupOption(
        collaborationNames,
        saved.collaborationType ?? saved.cooperation_type
      )
    );
    setExplanation(saved.explanation || saved.cooperation_description || "");
  }, [lookupsLoading, educationLevels, collaborationNames]);
  ...
  useEffect(() => {
    ...
    setCities(list);
    const saved = loadStep("salamtyaran", 2);
    const cityVal = saved?.city?.value ?? saved?.city;
    if (cityVal && saved?.province === province.value) {
      setCity(list.find((c) => c.value === cityVal) || null);
    } else if (cityVal) {
      setCity(list.find((c) => c.value === cityVal) || null);
    } else {
      setCity(null);
    ...
    setTimeout(() => {
      navigate("/lastsignup", {
        state: {
          ...
          previousStep: "/SalamtyaranSignupIndividual",
        },
      });
    }, 800);
  };
...
export default SalamatyaranSignupIndividual;
