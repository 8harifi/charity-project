...
import { Users, Stethoscope, Shield, Send } from "lucide-react";
import RenderDropdown from "../../pages/Auth/components/DropDown";

const Messages = () => {
  const [section, setSection] = useState("socialWorkers");
  ...
  const socialWorkers = ["مددکار احمدی", "مددکار رضایی"];
  const doctors = ["دکتر رضایی", "دکتر احمدی"];
  ...
  const getOptions = () => {
    if (section === "socialWorkers") return socialWorkers;
  ...
  const simulateIncomingMessage = (receiverName, type) => {
    ...
    const replies = {
      socialWorkers: "پرونده بیمار بررسی شد",
      doctors: "نتیجه آزمایش آماده است.",
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-2xl shadow border border-slate-100 space-y-5 max-w-4xl mx-auto">
      ...
      <div className="flex gap-2 overflow-x-auto">
        {tabButton("socialWorkers", "مددکاران", <Users size={18} />)}
        {tabButton("doctors", "پزشکان", <Stethoscope size={18} />)}
